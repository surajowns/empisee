	...
	<div class="card ctm-border-radius shadow-sm grow">
		...
		<div class="card-body">
			<a href="javascript:void(0)" class="avatar">
												<img class="profile_img" alt="avatar image" src="@if($employee['profile_image']) {{url('public/profile/'.$employee['profile_image'])}} @else {{url('public/assets/img/profiles/profile.png')}} @endif" class="img-fluid">
												<span class="profile_name">{{substr($employee['name'], 0, 2)}}</span>
											</a>
		</div>
	</div>
				...
				<div class="tab-pane fade show active" id="v-pills-home" role="tabpanel" aria-labelledby="v-pills-home-tab">
					<div class="table-responsive">
						<table class="table admin-table">
							...
							<tbody>
								@foreach($side_bar as $value)
								<tr>
									<td>
										<div>
											<h6 class="mb-0">{{$value['name']}}</h6>
											<p class="ctm-text-sm">{{$value['description']}}</p>
										</div>
									</td>
									<td>
										<div class="custom-control custom-switch">
											<input type="checkbox" class="custom-control-input permission" id="view{{$value['id']}}" data-emp_id={{$emp_id}} value="{{isset($value['sidebar_permission'])?$value['id']:$value['id']}}" {{isset($value['sidebar_permission'])?'checked':''}}>
											<label class="custom-control-label" for="view{{$value['id']}}"></label>
										</div>
									</td>
									<!-- <td>
																<div class="custom-control custom-switch">
																	<input type="checkbox" class="custom-control-input permission" id="manage{{$value['id']}}"  data-emp_id={{$emp_id}} onclick="confirm('Do you want to change the Permission?')"  value="{{isset($value['sidebar_permission'])?$value['id']:$value['id']}}" {{isset($value['sidebar_permission'])?'checked':''}}>
																	<label class="custom-control-label" for="manage{{$value['id']}}"></label>
																</div>
															</td> -->
								</tr>
								@endforeach
								@foreach($other_permission as $value)
								<tr>
									<td>
										<div>
											<h6 class="mb-0">{{$value['name']}}</h6>
									...
									<td>
										<div class="custom-control custom-switch">
											<input type="checkbox" class="custom-control-input permission" id="view{{$value['id']}}" data-emp_id={{$emp_id}} onclick="confirm('Do you want to change the Permission?')" value="{{isset($value['sidebar_permission'])?$value['id']:$value['id']}}" {{isset($value['sidebar_permission'])?'checked':''}}>
											<label class="custom-control-label" for="view{{$value['id']}}"></label>
										</div>
									</td>
									<!-- <td>
