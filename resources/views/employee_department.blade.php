@extends('master')
@section('title','Employee Deparment')
@section('content')
<?php
$id = Auth::user()->id;
...
<div class="col-xl-9 col-lg-8  col-md-12">
	...
	<div class="card shadow-sm grow ctm-border-radius">
		<div class="card-body align-center">
			<h4 class="card-title float-left mb-0 mt-2">{{count($data)}} Department</h4>
			<h6 class="error"></h6>
	...
	<div class="modal-dialog modal-dialog-centered">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<button type="button" class="btn btn-danger ctm-border-radius float-right ml-3" data-dismiss="modal">Cancel</button>
				<button type="button" class="btn btn-theme button-1 ctm-border-radius text-white float-right change_department" data-department_id="{{$value->id}}" data-department_name="{{$value->name}}">Save</button>
			</div>
		</div>
	</div>
</div>
...
<script>
	$(document).ready(function() {
		...
		$(document).on('click', '.change_department', function() {
			...
			$.ajax({
				Type: "GET",
				url: '{{url("change_department_name")}}',
				dataType: 'json',
				cache: false,
				data: {
