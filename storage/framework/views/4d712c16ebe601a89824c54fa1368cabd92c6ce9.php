<?php $__env->startSection('title','Employee Deparment'); ?>
...
<div class="col-xl-9 col-lg-8  col-md-12">
	...
	<div class="card shadow-sm grow ctm-border-radius">
		<div class="card-body align-center">
			<h4 class="card-title float-left mb-0 mt-2"><?php echo e(count($data)); ?> Department</h4>
			<h6 class="error"></h6>
			<ul class="nav nav-tabs float-right border-0 tab-list-emp">
				<li class="nav-item pl-3">
					<a href="javascript:void(0)" class="btn btn-theme button-1 text-white ctm-border-radius p-2 add-person ctm-btn-padding" data-toggle="modal" data-target="#addTeam"><i class="fa fa-plus"></i> Add Deparment</a>
				</li>
			</ul>
		</div>
	</div>
	<div class="row">
		...
		<div class="col-md-6">
			<div class="ctm-border-radius shadow-sm grow card">
				...
				<div class="card-body">
					<span class="avatar" data-toggle="tooltip" data-placement="top" title="" data-original-title=""><img src="assets/img/profiles/img-6.jpg" alt="Maria Cotton" class="img-fluid"></span>
	...
	<div class="modal-dialog modal-dialog-centered">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<h4 class="modal-title mb-3">Create New Deparment</h4>
				<form action="" id="addepartment">
					<div class="form-group">
						<input type="text" class="form-control" name="name" id="deparment" placeholder="Name" required>
						<span class="text-danger error"></span>
					</div>

					<button type="button" class="btn btn-danger ctm-border-radius float-right ml-3" data-dismiss="modal">Cancel</button>
					<button type="button" class="btn btn-theme button-1 ctm-border-radius text-white float-right" id="add_department">Add</button>
				</form>
			</div>
		</div>
	</div>
...
<div class="modal fade" id="edit<?php echo e($value->id); ?>">
	<div class="modal-dialog modal-dialog-centered">
		<div class="modal-content">
			...
			<div class="modal-body">
				<button type="button" class="close" data-dismiss="modal">&times;</button>
				<h4 class="modal-title mb-3">Edit Deparment</h4>
				<div class="form-group">
					<input class="form-control" type="text" name="name" value="<?php echo e($value->name); ?>">
				</div>
				<button type="button" class="btn btn-danger ctm-border-radius float-right ml-3" data-dismiss="modal">Cancel</button>
				<button type="button" class="btn btn-theme button-1 ctm-border-radius text-white float-right change_department" data-department_id="<?php echo e($value->id); ?>" data-department_name="<?php echo e($value->name); ?>">Save</button>
			</div>
		</div>
	</div>
</div>
...
<script>
	$(document).ready(function() {
		...
		$(document).on('click', '#add_department', function() {
			var name = $('#deparment').val();
			$.ajax({
				Type: "GET",
				url: '<?php echo e(url("add_department")); ?>',
				dataType: 'json',
				cache: false,
				data: {
					name: name
		...
		$(document).on('click', '.change_department', function() {
			...
			$.ajax({
				Type: "GET",
				url: '<?php echo e(url("change_department_name")); ?>',
				dataType: 'json',
				cache: false,
				data: {
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\empisee\resources\views/employee_department.blade.php ENDPATH**/ ?>